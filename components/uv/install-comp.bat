...
rem ================================================================
rem Script:  install-comp.bat (uv)
rem Purpose: Install uv on Windows, preferring winget.
...
set "OUT_FILE=%TEMP%\lanren-uv-install-%LOG_ID%.log"

net session >nul 2>&1
if %errorlevel% neq 0 (
    echo Requesting administrative privileges to install uv...
