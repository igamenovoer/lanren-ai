...
rem ================================================================
rem Script:  install-comp.bat (Tavily MCP)
rem Purpose: Install the Tavily MCP server via npm.
...
set "OUT_FILE=%TEMP%\lanren-tavily-mcp-install-%LOG_ID%.log"

net session >nul 2>&1
if %errorlevel% neq 0 (
    echo Requesting administrative privileges to install Tavily MCP server...
