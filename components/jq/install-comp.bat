...
rem ================================================================
rem Script:  install-comp.bat (jq)
rem Purpose: Install jq on Windows, preferring winget.
...
set "OUT_FILE=%TEMP%\lanren-jq-install-%LOG_ID%.log"

net session >nul 2>&1
if %errorlevel% neq 0 (
    echo Requesting administrative privileges to install jq...
