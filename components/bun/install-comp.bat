...
rem ================================================================
rem Script:  install-comp.bat (Bun)
rem Purpose: Install the Bun JavaScript runtime on Windows.
rem Usage:   install-comp.bat [--proxy <URL>] [--yes] [--from-official]
rem Notes:   Elevates to admin if needed and calls install-comp.ps1.
rem ================================================================
...
set "OUT_FILE=%TEMP%\lanren-bun-install-%LOG_ID%.log"

net session >nul 2>&1
if %errorlevel% neq 0 (
    echo Requesting administrative privileges to install Bun...
